...
class IndebtedMembersController extends Controller
{
    public function index()
    {
        return view('system.admin.pages.indebted_members.indebted_member');
    }
    ...
    public function getIndebtedMembers(Request $request)
    {
        if ($request->ajax()) {
            $data = Invoice::with('member')->where('remaining','!=',0)->latest()->get();
            $data = $data->unique('member_id');
            foreach($data as $item)
            {
                $item->member_remainig = Invoice::where('member_id',$item->member_id)->sum('remaining');
            }
